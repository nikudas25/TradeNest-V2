...
export function SellPage() {
    ...
    async function handleListingSubmit(event) {
        ...
        Object.keys(payload).forEach((key) => {
            ...
            if (typeof payload[key] === "object") {
                ...
                formData.append(key, Number(payload[key])); // ✅ force number
            } else {
        ...
        try {
            if (listingForm.id) {
                await api.updateListing(listingForm.id, formData);
    ...
    if (!user) {
        return (
            <div className="container empty-panel">
                ...
                <p>Create a buyer account first, then activate your seller workspace to post escrow-protected
                    listings.</p>
        ...
        <div className="page-stack container">
            ...
            <div className="account-layout">
                ...
                <form className="detail-card" onSubmit={handleListingSubmit}>
                    ...
                    <select onChange={(event) => handleListingChange("category_id", event.target.value)}
                    ...
                    <select onChange={(event) => handleListingChange("brand_id", event.target.value)}
                            value={listingForm.brand_id}>
                    ...
                    <input onChange={(event) => handleListingChange("price", event.target.value)}
                           placeholder="Price" type="number" value={listingForm.price}/>
                    <input
                    ...
                    />
                    <select onChange={(event) => handleListingChange("condition", event.target.value)}
            ...
            <div className="hero-actions">
                ...
                {listingForm.id ? (
                    <button className="button button--ghost" onClick={() => setListingForm(initialListingForm)}
                            type="button">
                        Cancel edit
                    </button>
                ) : null}
    ...
    <section className="detail-card">
        <h2>Your listings</h2>
        {!listings.length ?
            <p className="helper-copy">No listings yet. Publish your first item above.</p> : null}
        <div className="order-list">
            {listings.map((listing) => (
                <article className="order-card" key={listing.id}>
                    ...
                    <div className="hero-actions">
                        <button className="button button--secondary" onClick={() => startEditing(listing)}
                                type="button">
                            Edit
                        </button>
                        <button className="button button--ghost" onClick={() => deleteListing(listing.id)}
                                type="button">
                            Delete
                        </button>
                    </div>
        ...
        <div className="order-list">
            {sellerOrders.map((order) => (
                <article className="order-card" key={order.order_number}>
                    ...
                    <p>
                        Buyer placed order
                        on {formatDate(order.placed_at)} for {formatCurrency(order.grand_total)}
                    </p>
                    <div className="summary-row">
                        <span>Status</span>
                    ...
                    {order.status === "awaiting_shipment" ? (
                        <button className="button button--secondary"
                                onClick={() => shipOrder(order.order_number)} type="button">
                            Mark shipped
                        </button>
                    ) : null}
                </article>
